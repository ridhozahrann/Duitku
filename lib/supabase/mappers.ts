...
export function rowToBill(r: any) { return { id: r.id, name: r.name, amount: Number(r.amount), category: r.category, dueDate: new Date(r.due_date), recurrence: r.recurrence ?? undefined, status: r.status, notes: r.notes ?? undefined, createdAt: new Date(r.created_at), updatedAt: new Date(r.updated_at) } }
export function billToRow(b: any, userId: string) { return { id: b.id, user_id: userId, name: b.name, amount: b.amount, category: b.category, due_date: b.dueDate instanceof Date ? b.dueDate.toISOString() : new Date(b.dueDate).toISOString(), recurrence: b.recurrence ?? null, status: b.status, notes: b.notes ?? null } }
