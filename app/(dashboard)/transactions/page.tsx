...
export default function TransactionsPage() {
  ...
  const hasFilter = typeFilter !== 'all' || catFilter !== 'all' || !!search
  ...
  return (
    <div className="py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        {hasFilter && <Button variant="ghost" onClick={()=>{setSearch(''); setTypeFilter('all'); setCatFilter('all')}}>Reset</Button>}
      </div>

      {filtered.length ? (
        <Card><CardContent className="p-0"><div className="divide-y divide-gray-100 dark:divide-zinc-800">{filtered.map(t => <TransactionItem key={t.id} transaction={t} showTime />)}</div></CardContent></Card>
      ) : (
